...
void Teleport(DWORD X, DWORD Y){
	DWORD ecx = 0x029313DC;
	ecx = get(ecx);
	if(!ecx){
		return;
	}
	_Encrypt1(ecx + 0x113D0, 0x01);
	_Encrypt4(ecx + 0x113E8 + 0x0C, X);
	_Encrypt4(ecx + 0x113E8, Y);
}
...
void _declspec(naked) AutoPassword2(){
	_asm{
		...
		mov dword ptr [eax],0x37373737//Password2
		mov dword ptr [eax+0x4],0x00003737//Password2
		push eax
		movzx eax,[bMagica]
		mov dword ptr [esi+0x434],eax//Offline
		pop eax
		Return:
...
void magica(){
	...
	*(DWORD *)0x00EEB26C = 	(DWORD)AutoPassword2;
	...
	set(&_Encrypt1, 0x00A59DF0);
	set(&_Encrypt4, 0x0082C290);
