...
void magica(){
	...
	Frame *Client = (Frame *)(get(get(Maple.get() + 0x01)));
	...
	//Item Get Log (NEED TO UPDATE)
	Maple.scan("55 8D 6C 24 ?? 83 EC ?? 6A FF 68 ?? ?? ?? ?? 64 A1 00 00 00 00 50 81 EC ?? ?? ?? ?? A1 ?? ?? ?? ?? 33 C5 89 45 ?? 53 56 57 50 8D 45 ?? 64 A3 00 00 00 00 8B F1 33 DB 89 5D ?? E8 ?? ?? ?? ?? 85 C0 0F 84", "C2 04 00");

	//NPC talk range
	Maple.scan("75 ?? 8B 47 04 8B 0F 50 51 8D 54 24 ?? 52 FF 15 ?? ?? ?? ?? 85 C0 75 ?? 8B 47 04 8B 0F 50 51 8D 54 24 ?? 52 FF 15", "EB");

	//NPC talk speed (NEED TO UPDATE)
	Maple.scan("0F 85 ?? ?? ?? ?? 8B 4C 24 ?? 83 EC 08 8B C4 89 48 04 8B 44 24 ?? 89 64 24 ?? 85 C0 74", "90 90 90 90 90 90");
	...
	//Android (NEED TO UPDATE)
	Maple.scan("6A FF 68 ?? ?? ?? ?? 64 A1 00 00 00 00 50 83 EC ?? 56 A1 ?? ?? ?? ?? 33 C4 50 8D 44 24?? 64 A3 00 00 00 00 8B F1 E8 ?? ?? ?? ?? 0F B6 C0 50 E8");//AndroidTalk
