...
//	return IsDlgButtonChecked(hWndAW, nIDDlgItem);
	return IsDlgButtonChecked(hWndAW, nIDDlgItem) == 0 ? false : true;
}
...
void gui::SetString(int nIDDlgItem, LPCSTR lpString){
	SetDlgItemTextA(hWndAW, nIDDlgItem, lpString);
}

void gui::GetString(int nIDDlgItem, char *s){
	int StringSize = SendDlgItemMessageA(hWndAW, nIDDlgItem, WM_GETTEXTLENGTH, NULL, NULL);
	GetDlgItemTextA(hWndAW, nIDDlgItem, s, StringSize + 1);
}
...
// bien start
void list::MakeItemList(int itemCnt, ...)
{
    va_list arg;
    va_start(arg, itemCnt);

	this->list_item.item.clear();
	for(int index = 0; index < itemCnt; index++)
	{
		char* item= va_arg(arg, char*);
		this->list_item.item.insert(this->list_item.item.begin() + index, item);
	}
    va_end(arg);
	this->rows.push_back(this->list_item);
}
// bien end
...
void list::GetSelectedItem(int Index, char *s){
	...
	ListView_GetItemText(hWndAL, SelectLine, Index, s, MaxStringAL);
}
