...
DWORD HackShield_Flag = 0;
DWORD MODOR2(DWORD dwKey, BYTE Output[], DWORD *dwUnknown){
	WORD *Next = (WORD *)(&Output[8]);
	*(DWORD *)(&Output[*Next]) = ~((HackShield_Flag & 0xFFFF)* (dwKey & 0xFFFF)) % 0xA8CFA835;//false
	*Next += 4;
	*(DWORD *)(&Output[*Next]) = ~((HackShield_Flag >> 16)* (dwKey & 0xFFFF)) % 0xA8CFA835;//true
	*Next += 4;
	*dwUnknown += 4;//wtf
...
void HackShieldBypass(){
	...
	HM.WriteHook(0xC4930, JMP, (void (*)())MODOR2);//ModOR2

	HM.MemoryWriter(0x19000, "31 C0 C3");//HS Log
	int3bp_Bypass();
}