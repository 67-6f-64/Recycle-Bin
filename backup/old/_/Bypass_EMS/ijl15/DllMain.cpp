...
#include"MultiMS.h"
//code start
WORD Header_Login = 0x003E;
WORD Header_Connect = 0x0022;
BYTE HWID[10];

void CreateNewHWID(BYTE *Packet){
	int i;
	WORD wMapleIDSize = *(WORD *)(&Packet[25]);
	char *MapleID = (char *)(&Packet[27]);

	ZeroMemory(HWID, 10);
	RtlCopyMemory(HWID, &Packet[8], 10);
...
void HWIDBypass(){
	Air::WriteJump(HookAddr, (DWORD)PacketHook, &HookRet);
}
...
void MSCRCBypass(){
	...
	DWORD MSCRC23 = Air::AobScan("55 8B EC 6A FF 68 ?? ?? ?? ?? 64 A1 00 00 00 00 50 81 EC ?? ?? ?? ?? 53 56 57 A1 ?? ?? ?? ?? 33 C5 50 8D 45 F4 64 A3 00 00 00 00 89 8D ?? ?? ?? ?? 6A 01 8B 8D ?? ?? ?? ?? E8");
	...
	*(DWORD *)(MSCRC1 + 2) = (DWORD)MSCRC1_Hook - MSCRC1 - 6;
	...
	Air::WriteCode(MSCRC23, "31 C0 C3");
}
