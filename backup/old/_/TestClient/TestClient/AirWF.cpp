...
//15:52 2014/08/06
void _stdcall AirWindow::_SetFormatString(int nIDDlgItem, char *_Format, DWORD *dwArgument){
	char temp[MaxString] = {0};
	Air::_CreateFormatString(temp, _Format, dwArgument);
	SetString(nIDDlgItem, temp);
}


void _stdcall AirWindow::SetFormatString(int nIDDlgItem, char *_Format, ...){
	_SetFormatString(nIDDlgItem, _Format, (DWORD *)(&_Format+1));
}
