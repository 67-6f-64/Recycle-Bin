...
HWND Airgui::SetCheck(HWND hwnd, int INDEX, char Caption[], int x, int y, int nWidth, int nHeight){
	...
	if(nWidth == 0){
		for(fWidth = 0; Caption[fWidth]!=0x00; fWidth++){
		}
		fWidth *= 6;
		fWidth +=20;
	}

		newhwnd = CreateWindowExA(NULL, "BUTTON", Caption, WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, x, y, fWidth, nHeight, hwnd, (HMENU)INDEX, (HINSTANCE)GetWindowLongA(hwnd, GWL_HINSTANCE), NULL);
		SetFont(newhwnd);
		//SetBkMode(GetDC(newhwnd), TRANSPARENT);
	return newhwnd;
}
...
HWND Airgui::SetStatic(HWND hwnd, int INDEX, char Caption[], int x, int y, int nWidth, int nHeight){
	...
	if(nWidth == 0){
		...
		newhwnd = CreateWindowExA(NULL, "STATIC", Caption, WS_CHILD | WS_VISIBLE | SS_RIGHT, x, y, fWidth, nHeight, hwnd, (HMENU)INDEX, (HINSTANCE)GetWindowLongA(hwnd, GWL_HINSTANCE), NULL);
		SetFont(newhwnd);
	return newhwnd;
}
...
HWND Airgui::SetNumberEdit(HWND hwnd, int INDEX, char Caption[], int x, int y, int nWidth, int nHeight){
	HWND newhwnd;
		newhwnd = CreateWindowExA(NULL, "EDIT", Caption, WS_CHILD | WS_VISIBLE | ES_LEFT | ES_AUTOHSCROLL | WS_BORDER | ES_NUMBER, x, y, nWidth, nHeight, hwnd, (HMENU)INDEX, (HINSTANCE)GetWindowLongA(hwnd, GWL_HINSTANCE), NULL);
		SetFont(newhwnd);
	return newhwnd;
}

HWND Airgui::SetMultiLineEdit(HWND hwnd, int INDEX, char Caption[], int x, int y, int nWidth, int nHeight){
	HWND newhwnd;
		newhwnd = CreateWindowExA(NULL, "EDIT", Caption, WS_CHILD | WS_VISIBLE | ES_LEFT | ES_AUTOHSCROLL | WS_BORDER | ES_MULTILINE, x, y, nWidth, nHeight, hwnd, (HMENU)INDEX, (HINSTANCE)GetWindowLongA(hwnd, GWL_HINSTANCE), NULL);
		SetFont(newhwnd);
	return newhwnd;
}