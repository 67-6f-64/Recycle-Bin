...
DWORD dwSendHookAddr = 0x016B8E69;
DWORD dwSendHookRet = 0x01B19849;
DWORD dwCommonReturn = 0x0048AC30;
...
DWORD dwRecvHookAddr = 0x01AD6BC0;
DWORD dwRecvHookRet = 0x018C3619;

void _declspec(naked) RecvHook(){
	_asm{
		pushad
		mov ebx,[ebp+0x08]//RecvStrcut
		cmp word ptr [ebx+0x0C],0x0006
		jb EndingR
...
void WriteHook(){
	...
	*(DWORD *)(dwSendHookAddr+1) = (DWORD)SendHook - dwSendHookAddr - 5;
	...
	VirtualProtect((DWORD *)dwRecvHookAddr, 5, PAGE_EXECUTE_READWRITE, &old);

	*(BYTE *)(dwRecvHookAddr) = 0xE9;
	*(DWORD *)(dwRecvHookAddr+1) = (DWORD)RecvHook - dwRecvHookAddr - 5;
...
void AirPE(HINSTANCE hInstance){
	AW.CreateSimpleWindow(hInstance, "AirPE TWMS", 800, 600, GuiCreationI, GuiControlI, GuiNotifyI, true);
	WriteHook();
	Air::WaitForQuit();
}
