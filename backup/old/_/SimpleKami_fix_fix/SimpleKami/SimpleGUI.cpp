...
void GUI::CreateControls(HWND hwnd){
	...
	Airgui::SetNumberEdit(hwnd, EDIT_CCval, "300", 5 + 6*12, 3 + 20*2, 50);
	...
	Airgui::SetCheck(hwnd, CHECK_ItemFilter, "Item Filter", 250-(20 + 6*12), 3+ 20);
	Airgui::SetCheck(hwnd, CHECK_SSMouseFly, "SS MouseFly", 250-(20 + 6*12), 3+ 20*2);
}
  ...
  switch(msg)  {
	...
	case WM_COMMAND:
		switch(LOWORD(wParam)){
			...
			case CHECK_SSMouseFly:
				if(IsDlgButtonChecked(hwnd , LOWORD(wParam)) == BST_CHECKED){
					SCR2::SSMouseFly(TRUE);
				}else{
					SCR2::SSMouseFly(FALSE);
				}
				break;
...
void GUI::DialogInit(HINSTANCE hInstance){
	...
	Airgui::CreateNewDialog(hInstance, "Simple Kami v98.2", 250, 100, SimpleGUIProc);
