...
void _stdcall DupeMutex(HANDLE Mutex){
	HANDLE handle;
	DuplicateHandle(GetCurrentProcess(), Mutex, 0, &handle, 0, FALSE, DUPLICATE_CLOSE_SOURCE);
	CloseHandle(handle);
}

void _declspec(naked) MultiHS(){
	_asm{
		call eax
		pushad
		push eax
...
DWORD EHSvc_43F205 = 0x43F205;

DWORD CRCTable[10] = {0x927D8, 0x6B, 0x9D02C, 0xD7, 0x92AED, 0x023F, 0x5BF30, 0x50, 0, 0};

void _declspec(naked) ExecuteCRC(){
	_asm{
		mov eax,[esp]
		pushad
		xor edi,edi
		mov ecx,[EHSvc_157048]
NextTarget1:
		mov eax,[EHSvc_Start]
		sub dword ptr [ecx+edi*8],eax
		mov eax,Memory2
		add dword ptr [ecx+edi*8],eax
		inc edi
		cmp edi,4
		jb NextTarget1
		popad
...
NextTarget2:
		...
		jb NextTarget2
		popad
		jmp dword ptr [EHSvc_43F205]
	}
}

HANDLE hFakeProcess = 0;

HANDLE _stdcall FakeProcess(){
	return hFakeProcess;
}
...
void HackShieldBypass(){
	...
	CRCTable[0] += (DWORD)Memory2;
	CRCTable[2] += (DWORD)Memory2;
	CRCTable[4] += (DWORD)Memory2;
	CRCTable[6] += (DWORD)Memory2;
	...
	Write_code(EHSvc+0xCEB67, "B8 00 00 00 00");//MemoryProtection for x64
	
	//Hardware BreakPoint Detections
	Write_code(EHSvc+0xF550, "31 C0 C3");
	Write_code(EHSvc+0xF740, "31 C0 C3");
	Write_code(EHSvc+0xFED0, "31 C0 C2 18 00");
	Write_code(EHSvc+0x10E20, "31 C0 C3");

	EHSvc_43F205 += EHSvc;
	//Write_Hook("jmp", EHSvc+0x43F1FB, (DWORD)ExecuteCRC, 5);

	
	Write_code(EHSvc+0x4F5B0, "31 C0 C2 04 00");//Process
	Write_code(EHSvc+0x56380, "31 C0 C2 04 00");//Module
	Write_code(EHSvc+0x56400, "31 C0 C2 08 00");//err1
	Write_code(EHSvc+0x54800, "31 C0 C3");//err2
	Write_code(EHSvc+0x56220, "31 C0 C3");//err3
	Write_code(EHSvc+0x549E0, "31 C0 C3");//err4

	Write_code(EHSvc+0xCE786, "31 C0 C3");//err4

	Write_Hook("jmp", EHSvc_39ED7, (DWORD)HSCRC);

	//DRIVER ATTACK
	/*
	Write_code(EHSvc+0x4F475, "90 E9");
	Write_code(EHSvc+0x4F26F, "90 90 90 90 90 90");
	Write_code(EHSvc+0x4F28A, "90 90");
	Write_code(EHSvc+0x4F2A8, "EB");
	Write_code(EHSvc+0x4F2EF, "EB");
	Write_code(EHSvc+0x4F335, "90 90");
	Write_code(EHSvc+0x4F33A, "90 90");
	Write_code(EHSvc+0x4F348, "EB");
	Write_code(EHSvc+0x4F34F, "EB");
	Write_code(EHSvc+0x4F379, "90 90");
	Write_code(EHSvc+0x4F38B, "90 90 90 90 90 90");
	Write_code(EHSvc+0x4F39C, "EB");
	Write_code(EHSvc+0x4F3AC, "EB");
	Write_code(EHSvc+0x4F424, "90 90");
	Write_code(EHSvc+0x4F435, "90 90");
	
	Write_code(EHSvc+0x4F161, NULL, 0xCA);
	*/
	//

	//Write_code(EHSvc+0x4F140, "31 C0 C3");//Scan

	//Write_Hook("jmp", EHSvc+0x729B, (DWORD)MultiHS);
