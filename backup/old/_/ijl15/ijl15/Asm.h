...
namespace Asm{
	...
	void CodeCave(DWORD dwHookAddr, DWORD dwCodeSize, char opcode[], BOOL originalcode = TRUE);
	void StackHook(DWORD dwHookAddr, DWORD dwCodeSize, DWORD dwFunction, char opcode[], BOOL ebp = FALSE);
	void HookAPI2(char ModuleName[], char ProcName[], DWORD func, DWORD hikisuu, DWORD retval = 0);
}
