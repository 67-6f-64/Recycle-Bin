...
DWORD _stdcall SendLog(BYTE Packet[], DWORD dwSize, DWORD dwReturn){
	/*
	if(*(WORD *)&Packet[0] == 0x003E){
		*(WORD *)&Packet[7] = GetCurrentProcessId();
	}
	*/
	...
	if(dwSize < MaxPacketSize){
		...
		AL.AddFormatItem("@%w %xa", *(WORD *)&Packet[0], &Packet[2], 1024);
	}
	return 0;
}
...
void _stdcall RecvLog(BYTE Packet[], DWORD dwSize, DWORD dwReturn){
	...
	if(dwSize < MaxPacketSize){
		...
		AL.AddFormatItem("@%w %xa", *(WORD *)&Packet[4], &Packet[6], 1024);
	}
}
