...
void WriteHooks(){
	...
	SendFunction = AM.AobScan("55 8B EC 6A FF 68 ?? ?? ?? ?? 64 A1 00 00 00 00 50 81 EC ?? ?? ?? ?? A1 ?? ?? ?? ?? 33 C5 89 85 ?? ?? ?? ?? 53 56 57 50 8D 45 ?? 64 A3 00 00 00 00 89 8D ?? ?? ?? ?? 6A 00 E9");
	if(!SendFunction){
		SendFunction = AM.AobScan("55 8B EC 6A FF 68 ?? ?? ?? ?? 64 A1 00 00 00 00 50 83 EC ?? 53 56 57 A1 ?? ?? ?? ?? 33 C5 50 ?? ?? ?? 64 A3 00 00 00 00 ?? ?? ?? 6A 00 E9");
	}

	//CreateFileMappingA(INVALID_HANDLE_VALUE, NULL, PAGE_EXECUTE_READWRITE, 0, 10, "SendPacket");
	//HANDLE Symbol =  OpenFileMappingA(FILE_MAP_EXECUTE | FILE_MAP_READ | FILE_MAP_WRITE, 0, "SendPacket");
	//MapViewOfFile(Symbol, FILE_MAP_EXECUTE | FILE_MAP_READ | FILE_MAP_WRITE, 0, SendFunction, 10);
	...
	*/
	...
	FindJMP();
	FakeRet = AM.AobScan("90 90 90 90 90 90 90 90 90 90");
	AM.WriteHook(FakeRet, CALL, (void(*)())SendFunction);
	FakeRet += 5;
	AM.MemoryWriter(FakeRet, "90 C3");
}