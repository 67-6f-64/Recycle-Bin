...
void MapleStory(){
	...
	//xc less
	Maple.scan("6A FF 68 ?? ?? ?? ?? 64 A1 00 00 00 00 50 51 56 A1 ?? ?? ?? ?? 33 C4 50 8D 44 24 ?? 64 A3 00 00 00 00 8B F1 89 74 24 ?? C7 06 ?? ?? ?? ?? C7 44 24 14 ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B CE E8");
	Maple.write(Maple.get() + 0x0130, "31 C0 C2 04 00");
	//process list packet
